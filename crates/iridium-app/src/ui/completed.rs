...
pub fn show(ui: &mut Ui, state: &mut AppState) {
    if state.completed.is_empty() {
        ui.add_space(8.0);
        ui.label(
            egui::RichText::new("No completed jobs yet.")
            ...
            for (display_i, real_i) in (0..n).rev().enumerate() {
                ...
                body.row(22.0, |mut row| {
                    ...
                    let bytes = match &job.outcome {
                        Ok(JobOutcome::Completed { bytes_processed, .. }) => format_bytes(*bytes_processed),
                        Ok(JobOutcome::Cancelled { bytes_done }) => format!("~{}", format_bytes(*bytes_done)),
                        Ok(JobOutcome::Recovery { result, .. }) => format_bytes(result.finished_bytes),
                        Err(_) => "—".to_owned(),
                    };
                    ...
                    row.col(|ui| {
                        ui.label(
                            egui::RichText::new(&bytes).font(egui::FontId::monospace(11.0)),
                        );
                    });
                    row.col(|ui| {
                        result_chip(ui, job);
...
fn result_chip(ui: &mut Ui, job: &CompletedJob) {
    match &job.outcome {
        Err(e) => {
            ...
            ui.label(egui::RichText::new(e.to_string()).small().color(Palette::DANGER))
                .on_hover_text(e.to_string());
        }
        Ok(JobOutcome::Cancelled { .. }) => {
            theme::chip_warn(ui, "CANCELLED");
        }
        Ok(JobOutcome::Completed { verified, bad_chunks, .. }) => {
            if *bad_chunks > 0 {
...
fn show_job_detail(ui: &mut Ui, job: &CompletedJob) {
    egui::Grid::new("job_detail_grid")
        ...
        .spacing([8.0, 3.0])
        .show(ui, |ui| {
            ui.label(egui::RichText::new("Source").color(Palette::TEXT_DIM).small());
            ui.add(egui::Label::new(
    ...
    match &job.outcome {
        ...
        Ok(JobOutcome::Completed { digests, bytes_processed, bad_chunks, verified }) => {
            ...
            for d in digests {
                ui.horizontal(|ui| {
                    ui.label(egui::RichText::new(format!("{:?}", d.algorithm)).small().color(Palette::TEXT_DIM));
                    ui.add(egui::Label::new(
